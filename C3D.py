...
def C3D(weights='sports1M', n_classes=487):
    ...
    model.add(Dense(n_classes, activation='sigmoid', name='fc8'))

    return model


if __name__ == '__main__':
    input_shape = (20, 299, 299, 3)

    x = Input(shape=input_shape)
    model = C3D(weights='sports1M')
    # model = Model(inputs=x, outputs=y.output)
    # model = y.output
    print(model.summary())