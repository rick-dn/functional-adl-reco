...
def Inception_Inflated3d(include_top=True,
    ...
    if include_top:
        ...
        print('rem frames: ', num_frames_remaining, classes, x)
    ...
    if weights in WEIGHTS_NAME:
        ...
        model.load_weights(downloaded_weights_path, skip_mismatch=True)
...
def i3d_dense(include_top=False, weights=None, n_classes=18, input_shape=(30, 299, 299, 3)):

    x = Input(shape=input_shape)
    y = Inception_Inflated3d(include_top=include_top, weights=weights,
                             input_shape=input_shape)(x)
    # y = layer
    y = layers.Dense(units=n_classes, activation='softmax')(y)
    y = layers.Flatten()(y)
...
def i3d_dense_299(include_top=False, weights=None, n_classes=18, input_shape=(30, 299, 299, 3)):
...
def main():
    ...
    input_shape = (30, 299, 299, 3)
    i3d_dense_299(include_top=False,
                  weights=weights,
                  n_classes=n_classes,
