...
from net_fv import NetFV, NetFV_Spatial, NetFV_Temporal
...
def TCN_resnet(
    ...
    input_tensor = Input(batch_shape=(batch_size, *dim))
    # model = input
...
def tcn_resnet_spatial_temporal(batch_size, n_classes, dim=None,
    ...
    y_spatial = Lambda(lambda i: K.reshape(i, (batch_size * dim[0], 20, 3)))(x)
    print('y_spatial after lambda 1: ', y_spatial)
    y_spatial = tcn_resnet_1(dim=(20, 3),
                     kernel_regularizer=kernel_regularizer,
                     activation=activation)(y_spatial)

    print('y_spatial before lambda 2: ', y_spatial)
    y_spatial = Lambda(lambda i: K.reshape(i, (batch_size, dim[0], 1280)))(y_spatial)
    print('y_spatial after lambda 2: ', y_spatial)
    y_spatial = tcn_resnet_2(
        n_classes=n_classes,
        batch_size=batch_size,
    ...
    #     #                              strides=1)(y_spatial)
    #     y_spatial = AveragePooling1D()(y_spatial)
    ...
    y_temporal = tcn_resnet_1(dim=(60, 600),
                              kernel_regularizer=kernel_regularizer,
                              activation=activation)(y_temporal)
    ...
    #     y_temporal = AveragePooling1D()(y_temporal)
    ...
    y_temporal = NetFV_Temporal()(y_temporal)
...
def tcn_resnet_spatial_enc(batch_size, n_classes, dim=None,
                    gap=1, kernel_regularizer=None, activation=None):
    ...
    model = Model(inputs=x, outputs=y_spatial)
    # model.summary()
...
def main():
    ...
    # model = tcn_resnet_spatial_enc(batch_size=16, n_classes=n_classes, dim=(600, 60),
    #                                kernel_regularizer=reg, activation=activation)

    model = tcn_resnet_spatial_temporal(batch_size=16, n_classes=n_classes, dim=(600, 60),
                                        kernel_regularizer=reg, activation=activation)
